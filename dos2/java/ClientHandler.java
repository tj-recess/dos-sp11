...
public class ClientHandler implements Runnable 
{
	...
	private	static boolean firstCaterReaderCond = true;
...
//						System.out.println("DEBUG: Writer : requestNum = " + myRequestNum);
						...
						synchronized(Server.WRITE_CONDITION)
						{
							while(Server.isWriterActive() || Server.getActiveReadersCount() > 0 || Server.getWaitingReadersCount() > 0
									|| (firstCaterReaderCond && Server.getNumReaders() > 0))	//last condition ensures that if there is any expected 
							{													//reader then writer should first wait
								//wait on read condition until some writer notifies
//								System.out.println("DEBUG: Writer : waiting on write condition until someone notifies. i = " + i + ", numReaders = " + Server.getNumReaders() + ", activeReaders = " + Server.getActiveReadersCount() + ", waitingReaders = " + Server.getWaitingReadersCount());
//								System.out.println("firstCaterReaderCond = " + firstCaterReaderCond);
								if(Server.hasReaderArrived())
